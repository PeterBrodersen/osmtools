...
def get_all_osm_ids_with_gender_from_db(conn):
    ...
    This queries paris_osmetymology.locations_agg joined to paris_osmetymology.wikidata and
    left-joins paris_osmetymology.gendermap to obtain a gender (e.g. 'male'/'female')
    ...
    with conn.cursor() as cur:
        for geomtype, key in queries:
            cur.execute(
                ...
                INNER JOIN paris_osmetymology.wikidatamap map ON l.id = map.location_id
                INNER JOIN paris_osmetymology.wikidata w ON map.wikidata_id = w.itemid
                LEFT JOIN paris_osmetymology.gendermap g ON (w.claims->'P21'->0->'mainsnak'->'datavalue'->'value'->>'id') = g.itemid
...
def get_all_wikidata_descriptions(conn):
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            FROM paris_osmetymology.wikidata
